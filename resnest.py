...
from keras.layers import (
  Activation, Add, AveragePooling2D, BatchNormalization, Conv2D, Dense, Dropout,
  GlobalAveragePooling2D, GlobalMaxPooling2D, MaxPool2D, Reshape
)
...
WEIGHTS_HASHES = {
  'resnest50': {
    'url':
    'https://github.com/lucasdavid/resnest-tf/releases/download/v0.0.1/resnest50.h5',
    'hash': 'ebc1bb7cf7e960c9de7061e06c516fcb51885915ffede7869452c2180ec34759',
  },
  'resnest101': {
    'url':
    'https://github.com/lucasdavid/resnest-tf/releases/download/v0.0.1/resnest101.h5',
    'hash': '63b9202b4e6dc4019ea98cba649a36f6990afd194b0696d4a733729adffd017b',
  },
  'resnest200': {
    'url':
    'https://github.com/lucasdavid/resnest-tf/releases/download/v0.0.1/resnest200.h5',
    'hash': '9199062c60ff5d5f8248cd518321660c731af7e8b28f5ece6d86413aff63d94d',
  },
  'resnest269': {
    'url':
    'https://github.com/lucasdavid/resnest-tf/releases/download/v0.0.1/resnest269.h5',
...
def splat_block(
  ...
  if 1 < radix:
    ...
    y = tf.keras.layers.Add(name=f"{prefix}splat_out")([o * a for o, a in zip(split, a)])
  else:
...
def resnest_block(
  ...
  if avd:
    avd_layer = AveragePooling2D(
      3, strides=stride_size, padding="same", name=f"stage{stage}_block{index}_avd"
    )
    stride_size = 1
    if avd_first:
      y = avd_layer(y)
  ...
  y = Conv2D(n_filter * expansion, 1, name=f"stage{stage}_block{index}_conv3",
             **cv_args)(y)
  y = BatchNormalization(name=f"stage{stage}_block{index}_bn3", **bn_args)(y)
  if 0 < dropout_rate:
    y = Dropout(dropout_rate, name="stage{stage}_block{index}_dropout3")(y)
  residual = x
  ...
  if stride_size != 1 or tf.keras.backend.int_shape(x)[-1] != (n_filter * expansion):
    if avg_down:
      if dilation == 1:
        ...
        downsample = AveragePooling2D(
          1, strides=1, padding="same", name=f"stage{stage}_downsample_avgpool"
        )(x)
      downsample = Conv2D(
        n_filter * expansion, 1, name=f"stage{stage}_downsample_conv1", **cv_args
      )(downsample)
      downsample = BatchNormalization(name=f"stage{stage}_downsample_bn1",
                                      **bn_args)(downsample)
    else:
      downsample = Conv2D(
        n_filter * expansion,
...
def ResNet(
  ...
  if include_top:
    y = Dense(
      n_class, activation=classifier_activation, name='predictions', dtype='float32'
    )(y)

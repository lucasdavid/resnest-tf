...
from keras.layers import (
  ...
  Dropout, GlobalAveragePooling2D, GlobalMaxPooling2D, MaxPool2D,
  Reshape
)


CV_DEFAULTS = dict(padding='same', use_bias=False, kernel_initializer='he_normal')
BN_DEFAULTS = dict(axis=-1, momentum=0.9, epsilon=1e-5,)
...
def splat_block(
  ...
  if not cv_args: cv_args = CV_DEFAULTS
  if not bn_args: bn_args = BN_DEFAULTS
...
def resnest_block(
  ...
  y = Conv2D(n_filter * expansion, 1, name=f"stage{stage}_block{index}_conv3", **cv_args)(y)
  ...
  if stride_size != 1 or tf.keras.backend.int_shape(x)[-1] != (n_filter * expansion):
    if avg_down:
      if dilation == 1:
        ...
        downsample = AveragePooling2D(1, strides=1, padding="same", name=f"stage{stage}_downsample_avgpool")(x)
      downsample = Conv2D(n_filter * expansion, 1, name=f"stage{stage}_downsample_conv1", **cv_args)(downsample)
      downsample = BatchNormalization(name=f"stage{stage}_downsample_bn1", **bn_args)(downsample)
    else:
      downsample = Conv2D(n_filter * expansion, 1, strides=stride_size, name=f"stage{stage}_downsample_conv1", **cv_args)(x)
      downsample = BatchNormalization(name=f"stage{stage}_downsample_bn1", **bn_args)(downsample)
